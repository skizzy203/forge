...
BCD output format overhaul. The generated `.bcd.md` file is now self-explanatory — every field carries the exact question label visible in the questionnaire UI (Q1, Q2, Q10a, QG-A, etc.) so a reader or downstream agent can identify the source of any answer without cross-referencing the form. Formatting follows the CommonMark specification throughout.

### Changed — `buildBCD()` output format

- **Question labels on all fields.** Every field now emits `**Qn — LABEL**` on its own line, followed by the value. Previously: Core Business used English prose labels with no question numbers; Diagnostic Depth used raw `id.toUpperCase()` (e.g., `Q15:`, `Q21:` with no label text); Operator Edge Profile and Phase-Specific Context also lacked question references.

- **Document header as a metadata bullet list.** The bare key: value lines at the top of the document are now a CommonMark list (`- **Generated by:** …`, `- **Date:** …`, etc.) for clean rendering in any Markdown viewer.

- **Numbers split into its own section.** `Q10a–Q10d`, `Q11a–Q11b`, `Q12`, `Q13`, and optional `Q14`/`Q19` move from the Core Business section into a dedicated `## Numbers` section. Core Business now covers Q1–Q9 (narrative) exclusively.

- **Phase-Specific Context carries phase question labels.** Each phase entry now includes its canonical label (QS-A, QG-A, QM-A, QE-A, etc.) matching what the operator saw in the form.

- **Blank-line paragraph separation throughout.** Each labeled field block is its own CommonMark paragraph (blank line before and after), making the document easy to parse visually and programmatically.
...
| **v1.11.7 (pending user setup)** | Private GitHub `forge-workshops` repo, committed by `submission-created` function | Function code already structured to add this. Blocks on user creating the private repo + a fine-grained PAT scoped to it. |
