...
Forge is a Claude Code plugin (`v1.14.5`) that delivers business model optimization for workshop facilitators. The single user-facing command — `/forge:optimize` — routes across three modes, then runs a four-phase pipeline that ends with a written HTML intelligence report.
...
references/catalog.md        — 40-model scoring catalog
...
3. **Adaptive Model Chain** — Scores and selects from `references/catalog.md` (40 models); hard anchors enforce causal order; stops when 2 consecutive picks fall below 4.0 or at 25 models
...
| `references/catalog.md` | All 40 mental models with scoring metadata, prompt kernels, BCD triggers |
